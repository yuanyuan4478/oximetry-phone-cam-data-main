...
def _plot_subject(
    ...
    ax.plot(time_axis, preds, label="Linear regression", linewidth=1.4, color=LINEAR_COLOR)
    ax.axhline(threshold, color="#ff7f0e", linestyle="--", linewidth=1.0, label=f"Threshold {threshold:.0f}%")
    ...
    ax.set_ylabel("SpO₂ (%)")
    ax.set_title(f"Subject {subject_id}: Linear Regression vs Ground Truth")
    ax.set_ylim(60, 100)
    ...
    output_path = output_dir / f"linear_correctness_subject_{subject_id}.png"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Plot linear regression correctness against ground truth")
    parser.add_argument(
        "--subjects",
        nargs="*",
        default=SUBJECT_IDS,
...
def main() -> None:
    ...
    for subject_id in subjects:
        labels, preds = _loocv_predict(datasets, subject_id)
